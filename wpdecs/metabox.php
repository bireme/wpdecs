...
<script>
                ...
                $.get(webservice_url, {lang: lang, words: word}, function( data ){
                    ...
                    for(item in data.descriptors) {
                        ...
                        for(qualifier in content.qualifiers) {
                            ql += '<li class="qualifier"><input type="checkbox" id="' + content.qid[qualifier] + '" data-term-id="'+content.tree_id+"|"+item+'" value="'+qualifier+'"> ' + content.qualifiers[qualifier] + '</li>';
                        }
                        ql += "</ul>";

                        var ql_html = "<tr id='ql_"+count+"' style='display:none'><td class='qualifiers' colspan='5'>"+ql+"</td></tr>";

                        var html = $('#result_example').html();
                        html = html.replace(/see_qualifiers/,'see_qualifiers_'+count);
                        html = html.replace(/result_example_title/,'result_example_title_'+count);
                        html = html.replace(/result_example_definition/,'result_example_definition_'+count);
                        html = html.replace(/result_example_link/,'result_example_link_'+count);

                        // append result
                        $("#search_results").append("<tr class='row-result' data-mfn='"+mfn+"' data-id='"+content.tree_id+"|"+item+"'>"+html+"</tr>"+ql_html);

                        count += 1;
                    }

                    if( !data.descriptors || data.descriptors.length < 1 ) {
    ...
    function select_term(id, term, mfn) {
        ...
        qualifiers.each(function(){
            ...
            el += '<input type="hidden" name="wpdecs_terms['+id_composto+'][qid]['+$(this).val()+']" value="'+$(this).attr('id')+'">';
